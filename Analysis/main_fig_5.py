...
from my_code.mymegdataset import MyMEGDataset
from my_code.myeegdataset import MyEEGDataset
from my_code.mybenchmarkdataset import MyBenchmarkDataset
from my_code.mybetadataset import MyBetaDataset
...
import my_code.betapreprocess as betapreproc

from my_code.megpreprocess import bandpass_filter
...
data = snr_fft_eeg_pick.flatten()
counts, bins = np.histogram(data, bins=bins,normed=True)
bar_width = 0.8 * (bins[1] - bins[0])
b1= ax.bar(bins[:-1], counts,color=eeg_color,width=bar_width,
       align='edge',edgecolor=None,alpha=alpha)

xmin, xmax = plt.xlim()
x = np.linspace(xmin, xmax, 100)
p = norm.pdf(x, np.mean(data), np.std(data))
plt.plot(x, p, eeg_color, linewidth=1.5)
plt.axvline(np.mean(data), color=eeg_color, linestyle='--')


data = snr_fft_meg_pick.flatten()
counts, bins = np.histogram(data, bins=bins,normed=True)
b2 = ax.bar(bins[:-1], counts,color=meg_color,width=bar_width,
       align='edge',edgecolor=None,alpha=alpha)

xmin, xmax = plt.xlim() 
x = np.linspace(xmin, xmax, 100)
p = norm.pdf(x, np.mean(data), np.std(data))
plt.plot(x, p, meg_color, linewidth=1.5)
plt.axvline(np.mean(data), color=meg_color, linestyle='--')


data = snr_fft_benchmark_pick.flatten()
counts, bins = np.histogram(data, bins=bins,normed=True)
b3 = ax.bar(bins[:-1], counts,color=benchmark_color,width=bar_width,
       align='edge',edgecolor=None,alpha=alpha)

xmin, xmax = plt.xlim()
x = np.linspace(xmin, xmax, 100)
p = norm.pdf(x, np.mean(data), np.std(data))
plt.plot(x, p, benchmark_color, linewidth=1.5)
plt.axvline(np.mean(data), color=benchmark_color, linestyle='--')


data = snr_fft_beta_pick.flatten()
counts, bins = np.histogram(data, bins=bins,normed=True)
b4 = ax.bar(bins[:-1], counts,color=beta_color,width=bar_width,
       align='edge',edgecolor=None,alpha=alpha)

xmin, xmax = plt.xlim()
x = np.linspace(xmin, xmax, 100)
p = norm.pdf(x, np.mean(data), np.std(data))
plt.plot(x, p, beta_color, linewidth=1.5)
plt.axvline(np.mean(data), color=beta_color, linestyle='--')
...
lg = ax.legend([b1,b2,b4,b3],['EEG','OPM-MEG','Benchmark','Beta'])
...
# %%----------------------------------------fig 5C----------------------------------------
from my_code.meeganalysis import power_ratio
spec_snr = dict()
for dataset_key in datasets.keys():
    dataset = datasets[dataset_key]
    if dataset.ID == 'MEG':
    ...
    for d_pick in data_pick:
        ...
        for i,stim_f in enumerate(stim_freqs):
                r = power_ratio(snrs_sub_mean[i,:],f,stim_f,7)
                ratio_sub.append(r)
        ratios.append(ratio_sub)
    ...
    spec_snr[dataset.ID]['specs'] = specs
    spec_snr[dataset.ID]['snrs'] = snrs
